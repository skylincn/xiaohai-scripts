#!/usr/bin/env python3
"""
Standalone Image Upload Service for OpenClaw
Listens on port 18081, can be accessed directly or via nginx
"""
...
<head>
    ...
    <style>
        * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; }
        ...
        #pickBtn { display: block; width: 100%; padding: 40px 20px; border: 2px dashed #007AFF; border-radius: 12px; background: #f0f7ff; text-align: center; font-size: 17px; color: #007AFF; font-weight: 500; margin-bottom: 16px; cursor: pointer; }
        #pickBtn:active { background: #e0efff; }
        #fileInput { position: absolute; left: -9999px; }
        #preview { width: 100%; max-height: 250px; object-fit: cover; border-radius: 12px; margin-bottom: 16px; display: none; }
        #upBtn { display: block; width: 100%; padding: 16px; background: #007AFF; color: white; border: none; border-radius: 12px; font-size: 17px; font-weight: 600; cursor: pointer; }
        #upBtn:disabled { background: #ccc; }
        #upBtn:active:not(:disabled) { opacity: 0.8; }
        #status { margin-top: 16px; padding: 16px; border-radius: 12px; font-size: 15px; line-height: 1.5; display: none; }
...
<div class="wrap">
    <h1>📤 Upload Image</h1>
    <p class="desc">Auto-compress for OpenClaw</p>
...
(function() {
    ...
    pickBtn.addEventListener('click', function() { hide(); fileInput.click(); });
    ...
    fileInput.addEventListener('change', function() {
        if (fileInput.files && fileInput.files.length > 0) {
            ...
            reader.onerror = function() { show('Cannot read file', 'err'); };
...
def run_server(port=18081):
    ...
    print(f"Upload service running at http://0.0.0.0:{port}/")
    print(f"Also accessible at https://claw.skylin.cn/image-upload/ (via nginx)")
    try:
